...
class VentasA_View(QWidget, Ui_VentasA):
    ...
    def generar_venta(self):
        ...
        try:
            ...
            # Configuración inicial para una impresora de 58 mm (POS-58)
            ...
            # Formatear valores monetarios (limitar decimales si es necesario)
            subtotal_formateado = f"${subtotal:,.0f}".replace(",", ".")
            total_formateado = f"${total:,.0f}".replace(",", ".")
            
            pago_p = str(pago).strip()
            if "/" in pago_p:
                total_p = pago_p.split("/") 
                efectivo = float(total_p[0]) if total_p[0] else 0
                tranferencia = float(total_p[1]) if total_p[1] else 0
                pago_formateado = f"Ef: ${efectivo:,.0f}\nTr: ${tranferencia:,.0f}".replace(",", ".")
            else:
                pago_f = float(pago_p.replace(".", "").replace(",", "."))
                pago_formateado = f"${pago_f:,.0f}".replace(",", ".")
           
            descuento_formateado = f"${descuento:,.0f}".replace(",", ".")

            delivery_fee = float(delivery_fee)
            delivery_fee_formateado = f"${delivery_fee:,.0f}".replace(",", ".")

            # Ajustar dirección para 58mm (max ~25 chars por línea en fuente 18)
            direccion = client_address
            direccion_linea1 = direccion[:25]
            direccion_linea2 = direccion[25:50] if len(direccion) > 25 else ""
            direccion_linea3 = direccion[50:75] if len(direccion) > 50 else ""
            ...
            # Fuente para el cuerpo del ticket
            font_size = 18
            ...
            # Printer width y centrado (58mm suele ser 384 pixels)
            ...
            y += 10
            hDC.TextOut(x, y, "--------------------------------")  # 32 guiones
            y += line_height
            
            # Info de la factura
            hDC.TextOut(x, y, f"No. COT: {self.invoice_number}")
            y += line_height
            ...
            if direccion_linea2:
                ...
                y += line_height
            if direccion_linea3:
                hDC.TextOut(x + 40, y, direccion_linea3)
                y += line_height

            # Encabezado (ahora más simple ya que los detalles van en 2 líneas)
            y += line_height
            hDC.TextOut(x, y, "--------------------------------")
            y += line_height  
            ...
            # Totales
            hDC.TextOut(x, y, f"SUBTOTAL:   {subtotal_formateado.rjust(18)}")
            y += line_height
            ...
            y += line_height
            y += 5
            hDC.TextOut(x, y, f"METODO: {payment_method}")
            y += line_height
            if "/" in pago_p:
                for lp in pago_formateado.split("\n"):
                    hDC.TextOut(x, y, lp)
                    y += line_height
            else:
                hDC.TextOut(x, y, f"PAGO: {pago_formateado}")
                y += line_height
            ...
            # Espacio adicional al final para evitar que la cuchilla corte el texto
            for _ in range(10):
                y += line_height
                hDC.TextOut(x, y, " ")
            hDC.TextOut(x, y, ".") # Punto final para forzar el avance de papel
